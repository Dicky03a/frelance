            ...
            <div className="px-6 py-10 space-y-20">
                        ...
                        <section key={service.id} className="space-y-12">
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-4">
                                ...
                                <div className="flex gap-4 text-xs font-bold text-white/20 uppercase tracking-widest">
                                    <span className="flex items-center gap-2"><Rocket size={14} /> {t('fast_delivery')}</span>
                                    <span className="flex items-center gap-2"><ShieldCheck size={14} /> {t('secure_code')}</span>
                                </div>
                            </div>
                ...
                <Dialog open={!!selectedPackage} onOpenChange={(open) => !open && setSelectedPackage(null)}>
                    <DialogContent className="sm:max-w-[500px] bg-[#111118] border-white/10">
                        <DialogHeader>
                            <DialogTitle className="text-2xl font-bold">{t('confirm_order')}</DialogTitle>
                ...
                <section className="rounded-[40px] border border-white/7 bg-[#1c1c28] p-8 md:p-16 flex flex-col md:flex-row gap-12 items-center">
                    <div className="flex-1 space-y-6">
                        <h2 className="text-3xl font-bold text-white">{t('custom_solution_title')}</h2>
                    ...
                    <div className="flex-1 grid grid-cols-2 gap-4 w-full">
                        <div className="p-6 rounded-3xl bg-white/5 border border-white/5 space-y-2">
                            ...
                            <h4 className="font-bold text-white">{t('dedicated_support')}</h4>
                            <p className="text-xs text-white/30">{t('dedicated_support_desc')}</p>
                        </div>
                        <div className="p-6 rounded-3xl bg-white/5 border border-white/5 space-y-2">
                            <Zap className="text-amber-400 mb-2" size={24} />
                            <h4 className="font-bold text-white">{t('high_performance')}</h4>
                            <p className="text-xs text-white/30">{t('high_performance_desc')}</p>
                        </div>
                    </div>
                </section>
            </div>
